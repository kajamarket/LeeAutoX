              ...
              if (p.startsWith('- ')) {
                return (
                  <ul key={index} className={`list-disc list-inside pl-4 space-y-2 my-4 ${theme === 'dark' ? 'text-zinc-300' : 'text-slate-800'}`}>
                    {p.split('\n').map((li, i) => (
                      <li key={i} className="leading-relaxed">{renderParagraphText(li.replace(/^- /, '').replace(/^\*\*(.+?)\*\*: /, (_, bold) => bold + ': '))}</li>
                    ))}
                  </ul>
                );
              }
